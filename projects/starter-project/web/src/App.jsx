...
function App() {
  ...
  const testConnection = async () => {
    if (currentProvider.unsupported) {
      showToast(`${currentProvider.label}는 현재 연결 테스트를 지원하지 않습니다.`, "error");
      return;
    }
  ...
  const sendChat = async () => {
    if (currentProvider.unsupported) {
      showToast(`${currentProvider.label}는 현재 채팅을 지원하지 않습니다.`, "error");
      return;
    }
              ...
              {providers.map((p) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (p.unsupported) {
                        showToast(`${p.label}는 현재 미지원입니다.`, "error");
                        return;
                      }
                      setProviderId(p.id);
                    }}
